...
/// \file AgentHierarchy.cpp
/// \brief Definition of the AgentHierarchy class
...
unsigned AgentHierarchy::get_number_of_registered_types() {
  return number_of_registered_types_;
}

bool AgentHierarchy::is_valid(const std::string& name, const std::string& parent_name) {
  return get_name_pair_to_type_map().count(std::make_pair(name, parent_name)) != 0;
}

bool AgentHierarchy::is_valid(const agent_t agent_type) {
  return get_name_map().count(agent_type) != 0;
}

void AgentHierarchy::assert_is_valid(const std::string& name, const std::string& parent_name) {
...
void AgentHierarchy::assert_is_valid(const agent_t agent_type) {
...
agent_t AgentHierarchy::get_agent_type(const std::string& name, const std::string& parent_name) {
  assert_is_valid(name, parent_name);
  return get_name_pair_to_type_map()[std::make_pair(name, parent_name)];
}

std::string AgentHierarchy::get_name(const agent_t agent_type) {
  assert_is_valid(agent_type);
  return get_name_map()[agent_type];
}

std::string AgentHierarchy::get_parent_name(const std::string& name, const std::string& parent_name) {
  assert_is_valid(name, parent_name);
  const agent_t agent_type = get_agent_type(name, parent_name);
  return get_parent_name(agent_type);
}

std::string AgentHierarchy::get_parent_name(const agent_t agent_type) {
  assert_is_valid(agent_type);
  return get_parent_name_map()[agent_type];
}

stk::topology::topology_t AgentHierarchy::get_topology(const std::string& name, const std::string& parent_name) {
  assert_is_valid(name, parent_name);
  const agent_t agent_type = get_agent_type(name, parent_name);
  return get_topology(agent_type);
}

stk::topology::topology_t AgentHierarchy::get_topology(const agent_t agent_type) {
  assert_is_valid(agent_type);
  return get_topology_generator_map()[agent_type]();
}

stk::topology::rank_t AgentHierarchy::get_rank(const std::string& name, const std::string& parent_name) {
  assert_is_valid(name, parent_name);
  const agent_t agent_type = get_agent_type(name, parent_name);
  return get_rank(agent_type);
}

stk::topology::rank_t AgentHierarchy::get_rank(const agent_t agent_type) {
  assert_is_valid(agent_type);
  return get_rank_generator_map()[agent_type]();
}

void AgentHierarchy::add_part_reqs(std::shared_ptr<mundy::meta::PartRequirements> part_reqs_ptr,
...
void AgentHierarchy::add_subpart_reqs(std::shared_ptr<mundy::meta::PartRequirements> subpart_reqs_ptr,
                                      const std::string& name, const std::string& parent_name) {
  assert_is_valid(name, parent_name);
  const agent_t agent_type = get_agent_type(name, parent_name);
...
std::shared_ptr<mundy::meta::MeshRequirements> AgentHierarchy::get_mesh_requirements(const std::string& name,
...
std::shared_ptr<mundy::meta::MeshRequirements> AgentHierarchy::get_mesh_requirements(const agent_t agent_type) {
  assert_is_valid(agent_type);
...
void AgentHierarchy::print_hierarchy(std::ostream& os) {
  StringTreeManager::print_tree(os);
}

std::string AgentHierarchy::get_hierarchy_as_a_string() {
  std::stringstream ss;
  print_hierarchy(ss);
  return ss.str();
}
...
AgentHierarchy::StringTreeNode::StringTreeNode(const unsigned id, const std::string& name,
                                               const std::string& parent_name)
    : id_(id), name_(name), parent_name_(parent_name) {
}

void AgentHierarchy::StringTreeNode::add_child(const unsigned id, const std::string& name,
                                               const std::string& parent_name) {
  children_.push_back(std::make_shared<StringTreeNode>(id, name, parent_name));
}

void AgentHierarchy::StringTreeNode::add_child(std::shared_ptr<StringTreeNode> child) {
  children_.push_back(child);
}

unsigned AgentHierarchy::StringTreeNode::get_id() const {
  return id_;
}

std::string AgentHierarchy::StringTreeNode::get_name() const {
  return name_;
}

std::string AgentHierarchy::StringTreeNode::get_parent_name() const {
  return parent_name_;
}

std::shared_ptr<AgentHierarchy::StringTreeNode> AgentHierarchy::StringTreeNode::get_child(
...
std::vector<std::shared_ptr<AgentHierarchy::StringTreeNode>> AgentHierarchy::StringTreeNode::get_children() const {
  return children_;
}

std::shared_ptr<AgentHierarchy::StringTreeNode> AgentHierarchy::StringTreeManager::create_node(
  ...
  // Create the node.
  std::shared_ptr<AgentHierarchy::StringTreeNode> node =
      std::make_shared<AgentHierarchy::StringTreeNode>(id, name, parent_name);
...
void AgentHierarchy::StringTreeManager::print_tree(std::ostream& os) {
  for (const auto& pair : root_node_map_) {
    print_tree(pair.second, os);
  }
}

void AgentHierarchy::StringTreeManager::print_tree(std::shared_ptr<StringTreeNode> node, std::ostream& os, int depth) {
