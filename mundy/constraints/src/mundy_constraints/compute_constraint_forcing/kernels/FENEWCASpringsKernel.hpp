...
#ifndef MUNDY_CONSTRAINTS_COMPUTE_CONSTRAINT_FORCING_KERNELS_FENEWCASPRINGSKERNEL_HPP_
#define MUNDY_CONSTRAINTS_COMPUTE_CONSTRAINT_FORCING_KERNELS_FENEWCASPRINGSKERNEL_HPP_

/// \file FENEWCASpringsKernel.hpp
/// \brief Declaration of the ComputeConstraintForcing's FENEWCASpringsKernel kernel.
...
// Mundy libs
#include <mundy_constraints/FENEWCASprings.hpp>  // for FENEWCASprings
...
class FENEWCASpringsKernel : public mundy::meta::MetaKernel<> {
 public:
  ...
  /// \brief Constructor
  explicit FENEWCASpringsKernel(mundy::mesh::BulkData *const bulk_data_ptr,
  ...
  static std::shared_ptr<mundy::meta::MeshReqs> get_mesh_requirements(
    ...
    for (int i = 0; i < num_parts; i++) {
      ...
      if (part_name == FENEWCASprings::get_name()) {
        // Add the requirements directly to sphere sphere linkers agent.
        FENEWCASprings::add_and_sync_part_reqs(part_reqs);
      } else {
        // Add the associated part as a subset of the sphere sphere linkers agent.
        FENEWCASprings::add_and_sync_subpart_reqs(part_reqs);
      }
    }

    return FENEWCASprings::get_mesh_requirements();
  }
  ...
  static Teuchos::ParameterList get_valid_fixed_params() {
    static Teuchos::ParameterList default_parameter_list =
        Teuchos::ParameterList()
            .set("valid_entity_part_names", mundy::core::make_string_array(FENEWCASprings::get_name()),
  ...
  static std::shared_ptr<PolymorphicBaseType> create_new_instance(
      ...
      const Teuchos::ParameterList &fixed_params = Teuchos::ParameterList()) {
    return std::make_shared<FENEWCASpringsKernel>(bulk_data_ptr, fixed_params);
  }
...
#endif  // MUNDY_CONSTRAINTS_COMPUTE_CONSTRAINT_FORCING_KERNELS_FENEWCASPRINGSKERNEL_HPP_
