...
#ifndef MUNDY_CONSTRAINTS_HOOKEANSPRINGS_HPP_
#define MUNDY_CONSTRAINTS_HOOKEANSPRINGS_HPP_

/// \file HookeanSprings.hpp
/// \brief Declaration of the HookeanSprings part class
...
/// \class HookeanSprings
/// \brief The static interface for all of Mundy's HookeanSprings constraint.
///
...
class HookeanSprings {
 public:
  ...
  /// \brief Get the set of default field names for the HookeanSprings part.
  static inline std::vector<std::string> get_default_field_names() {
    return {std::string(node_coord_field_name_), std::string(element_hookean_spring_constant_field_name_),
            std::string(element_hookean_spring_rest_length_field_name_)};
  }

  /// \brief Get the default node coordinate field name for the HookeanSprings part.
  static inline std::string get_node_coord_field_name() {
    return std::string(node_coord_field_name_);
  }

  /// \brief Get the default element hookean spring constant field name for the HookeanSprings part.
  static inline std::string get_element_spring_constant_field_name() {
    return std::string(element_hookean_spring_constant_field_name_);
  }

  /// \brief Get the default element hookean spring rest length field name for the HookeanSprings part.
  static inline std::string get_element_rest_length_field_name() {
    return std::string(element_hookean_spring_rest_length_field_name_);
  }
 ...
 private:
  ...
  static constexpr std::string_view name_ = "HOOKEAN_SPRINGS";
  ...
  static inline std::shared_ptr<mundy::meta::PartRequirements> part_reqs_ptr_ = []() {
    ...
    part_reqs_ptr->add_field_reqs(std::make_shared<mundy::meta::FieldRequirements<double>>(
        std::string(element_hookean_spring_constant_field_name_), stk::topology::ELEMENT_RANK, 1, 1));
    part_reqs_ptr->add_field_reqs(std::make_shared<mundy::meta::FieldRequirements<double>>(
        std::string(element_hookean_spring_rest_length_field_name_), stk::topology::ELEMENT_RANK, 1, 1));
...
#endif  // MUNDY_CONSTRAINTS_HOOKEANSPRINGS_HPP_
