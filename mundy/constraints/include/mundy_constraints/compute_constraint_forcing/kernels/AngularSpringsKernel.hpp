...
class AngularSpringsKernel : public mundy::meta::MetaKernel<> {
 public:
  ...
  static std::shared_ptr<mundy::meta::MeshReqs> get_mesh_requirements(
    ...
    for (int i = 0; i < num_parts; i++) {
      ...
      if (part_name == AngularSprings::get_name()) {
        ...
        AngularSprings::add_and_sync_part_reqs(part_reqs);
      } else {
  ...
  static Teuchos::ParameterList get_valid_fixed_params() {
    ...
    default_parameter_list.set("valid_entity_part_names", mundy::core::make_string_array(AngularSprings::get_name()),
