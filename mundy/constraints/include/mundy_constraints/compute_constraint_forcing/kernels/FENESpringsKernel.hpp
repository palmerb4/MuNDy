...
#ifndef MUNDY_CONSTRAINTS_COMPUTE_CONSTRAINT_FORCING_KERNELS_FENESPRINGSKERNEL_HPP_
#define MUNDY_CONSTRAINTS_COMPUTE_CONSTRAINT_FORCING_KERNELS_FENESPRINGSKERNEL_HPP_

/// \file FENESpringsKernel.hpp
/// \brief Declaration of the ComputeConstraintForcing's FENESpringsKernel kernel.
...
// Mundy libs
#include <mundy_constraints/FENESprings.hpp>  // for FENESprings
...
/// \class FENESpringsKernel
/// \brief Concrete implementation of \c MetaKernel for computing the spring force induced by a collection of FENE
/// springs. constraint.
class FENESpringsKernel : public mundy::meta::MetaKernel<> {
 public:
  //! \name Typedefs
  //@{
  ...
  /// \brief Constructor
  explicit FENESpringsKernel(mundy::mesh::BulkData *const bulk_data_ptr,
  ...
  static std::shared_ptr<mundy::meta::MeshReqs> get_mesh_requirements(
    ...
    valid_fixed_params.validateParametersAndSetDefaults(FENESpringsKernel::get_valid_fixed_params());
    ...
    for (int i = 0; i < num_parts; i++) {
      ...
      if (part_name == FENESprings::get_name()) {
        // Add the requirements directly to sphere sphere linkers agent.
        FENESprings::add_and_sync_part_reqs(part_reqs);
      } else {
        // Add the associated part as a subset of the sphere sphere linkers agent.
        FENESprings::add_and_sync_subpart_reqs(part_reqs);
      }
    }

    return FENESprings::get_mesh_requirements();
  }
  ...
  static Teuchos::ParameterList get_valid_fixed_params() {
    static Teuchos::ParameterList default_parameter_list =
        Teuchos::ParameterList()
            .set("valid_entity_part_names", mundy::core::make_string_array(FENESprings::get_name()),
  ...
  static std::shared_ptr<PolymorphicBaseType> create_new_instance(
      ...
      const Teuchos::ParameterList &fixed_params = Teuchos::ParameterList()) {
    return std::make_shared<FENESpringsKernel>(bulk_data_ptr, fixed_params);
  }
 ...
 private:
  ...
  /// \brief Element field containing the spring's maximum extension.
  stk::mesh::Field<double> *element_rmax_field_ptr_ = nullptr;
...
#endif  // MUNDY_CONSTRAINTS_COMPUTE_CONSTRAINT_FORCING_KERNELS_FENESPRINGSKERNEL_HPP_
