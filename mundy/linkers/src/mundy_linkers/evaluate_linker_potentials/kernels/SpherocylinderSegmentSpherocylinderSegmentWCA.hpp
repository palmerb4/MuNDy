...
#ifndef MUNDY_LINKERS_EVALUATE_LINKER_POTENTIALS_SPHEROCYLINDERSEGMENTSPHEROCYLINDERSEGMENTWCA_HPP_
#define MUNDY_LINKERS_EVALUATE_LINKER_POTENTIALS_SPHEROCYLINDERSEGMENTSPHEROCYLINDERSEGMENTWCA_HPP_

/// \file SpherocylinderSegmentSpherocylinderSegmentWCA.hpp
/// \brief Declaration of the EvaluateLinkerPotentials's SpherocylinderSegmentSpherocylinderSegmentWCA
/// kernel.
...
/// \class SpherocylinderSegmentSpherocylinderSegmentWCA
/// \brief Concrete implementation of \c MetaKernel for computing the WCA contact force between two
/// spherocylinder_segments.
///
/// By definition the WCA contact force between two spherocylinder_segments can be calculated using the formula:
/// \f[
/// F = 24.0 * epsilon * (2.0 * rho12 - rho6)/(edge_length)
/// \f]
class SpherocylinderSegmentSpherocylinderSegmentWCA : public mundy::meta::MetaKernel<> {
 public:
  //! \name Typedefs
  //@{
  ...
  /// \brief Constructor
  explicit SpherocylinderSegmentSpherocylinderSegmentWCA(mundy::mesh::BulkData *const bulk_data_ptr,
  ...
  static Teuchos::ParameterList get_valid_mutable_params() {
    static Teuchos::ParameterList default_parameter_list =
        Teuchos::ParameterList()
            .set("epsilon", 1.0, "The epsilon parameter for the WCA potential.")
            .set("sigma", 1.0, "The sigma parameter for the WCA potential.")
            .set("lj_cutoff", 1.0, "The Lennard-Jones cutoff distance for the WCA potential.");
    return default_parameter_list;
  }
  ...
  static std::shared_ptr<PolymorphicBaseType> create_new_instance(mundy::mesh::BulkData *const bulk_data_ptr,
                                                                  const Teuchos::ParameterList &fixed_params) {
    return std::make_shared<SpherocylinderSegmentSpherocylinderSegmentWCA>(bulk_data_ptr, fixed_params);
  }
  //@}
  ...
  /// kernel.
  void execute(const stk::mesh::Selector &spherocylinder_segment_spherocylinder_segment_linker_selector) override;
  //@}
 ...
 private:
  ...
  /// \brief The default value for epsilon
  double epsilon_ = 1.0;

  /// \brief The default value for sigma
  double sigma_ = 1.0;

  /// \brief The default LJ cutoff distance for the WCA potential squared
  double lj_cutoff_ = 1.0;
  //@}
...
#endif  // MUNDY_LINKERS_EVALUATE_LINKER_POTENTIALS_SPHEROCYLINDERSEGMENTSPHEROCYLINDERSEGMENTWCA_HPP_
