...
SphereSpherocylinderSegmentLinker::SphereSpherocylinderSegmentLinker(mundy::mesh::BulkData *const bulk_data_ptr,
  ...
  auto field_exists = [](const stk::mesh::FieldBase *field_ptr, const std::string &field_name) {
    MUNDY_THROW_REQUIRE(field_ptr != nullptr, std::invalid_argument,
                        std::string("SphereSpherocylinderSegmentLinker: Field ") + field_name +
  ...
  auto parts_from_names = [](mundy::mesh::MetaData &meta_data, const Teuchos::Array<std::string> &part_names) {
    ...
    for (const std::string &part_name : part_names) {
      ...
      MUNDY_THROW_REQUIRE(part != nullptr, std::invalid_argument,
                          std::string("SphereSpherocylinderSegmentLinker: Part ") + part_name +
      ...
      [&node_coord_field, &element_radius_field, &linker_contact_normal_field, &linker_contact_points_field,
       &linker_signed_separation_distance_field, &linked_entities_field](
        ...
        MUNDY_THROW_ASSERT(bulk_data.is_valid(sphere_element), std::invalid_argument,
                           "SphereSpherocylinderSegmentLinker: sphere_element entity is not valid.");
        MUNDY_THROW_ASSERT(bulk_data.is_valid(spherocylinder_segment_element), std::invalid_argument,
                           "SphereSpherocylinderSegmentLinker: spherocylinder_segment_element entity is not valid.");
        ...
        // spherocylinder_segment_contact_point = sphere_contact_point + separation_distance * contact_normal;
        sphere_contact_point = sphere_center_coord;
        spherocylinder_segment_contact_point = closest_point;
      });
}
//}
