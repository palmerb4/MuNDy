...
#ifndef MUNDY_LINKERS_COMPUTE_SIGNED_SEPARATION_DISTANCE_CONTACT_NORMAL_AND_CONTACT_POINTS_SPHEROCYLINDERSPHEROCYLINDERLINKER_HPP_
#define MUNDY_LINKERS_COMPUTE_SIGNED_SEPARATION_DISTANCE_CONTACT_NORMAL_AND_CONTACT_POINTS_SPHEROCYLINDERSPHEROCYLINDERLINKER_HPP_

/// \file SpherocylinderSpherocylinderLinker.hpp
/// \brief Declaration of the ComputeSignedSeparationDistanceAndContactNormal's SpherocylinderSpherocylinderLinker
/// kernel.
...
/// \class SpherocylinderSpherocylinderLinker
/// \brief Concrete implementation of \c MetaKernel for computing the signed separation distance and contact normal
/// between two spherocylinders.
class SpherocylinderSpherocylinderLinker : public mundy::meta::MetaKernel<> {
 public:
  //! \name Typedefs
  //@{
  ...
  /// \brief Constructor
  explicit SpherocylinderSpherocylinderLinker(mundy::mesh::BulkData *const bulk_data_ptr,
  ...
  static std::shared_ptr<mundy::meta::MeshRequirements> get_mesh_requirements(
    ...
    for (int i = 0; i < num_linker_parts; i++) {
      ...
      part_reqs->add_field_reqs(std::make_shared<mundy::meta::FieldRequirements<double>>(
          linker_contact_points_field_name, stk::topology::CONSTRAINT_RANK, 6, 1));
  ...
  static std::shared_ptr<PolymorphicBaseType> create_new_instance(mundy::mesh::BulkData *const bulk_data_ptr,
                                                                  const Teuchos::ParameterList &fixed_params) {
    return std::make_shared<SpherocylinderSpherocylinderLinker>(bulk_data_ptr, fixed_params);
  }
  //@}
  ...
  /// \param spherocylinder_spherocylinder_linker_selector [in] The linker selector on by this kernel.
 ...
 private:
  ...
  /// \brief Element length field.
  stk::mesh::Field<double> *element_length_field_ptr_ = nullptr;

  /// \brief Element orientation field.
...
#endif  // MUNDY_LINKERS_COMPUTE_SIGNED_SEPARATION_DISTANCE_CONTACT_NORMAL_AND_CONTACT_POINTS_SPHEROCYLINDERSPHEROCYLINDERLINKER_HPP_
