...
KOKKOS_INLINE_FUNCTION Scalar poly_min_extrap(const Scalar f0, const Scalar d0, const Scalar f1, const Scalar d1,
                                              const Scalar limit = 1) {
  const Scalar n = 3 * (f1 - f0) - 2 * d0 - d1;
  const Scalar e = d0 + d1 - 2 * (f1 - f0);

  // find the minimum of the derivative of the polynomial

  const Scalar temp_sqr = Kokkos::max(n * n - 3 * e * d0, 0.0);
  if (temp_sqr < 0) {
    return 0.5;
  }
  if (Kokkos::abs(e) <= Kokkos::Experimental::epsilon_v<Scalar>) {
    return 0.5;
  }

  // figure out the two possible min values
  const Scalar temp = Kokkos::sqrt(temp_sqr);
  const Scalar x1 = (temp - n) / (3 * e);
  const Scalar x2 = -(temp + n) / (3 * e);

  // compute the value of the interpolating polynomial at these two points
  const Scalar y1 = f0 + d0 * x1 + n * x1 * x1 + e * x1 * x1 * x1;
  const Scalar y2 = f0 + d0 * x2 + n * x2 * x2 + e * x2 * x2 * x2;

  // pick the best point
  const Scalar x = (y1 < y2) ? x1 : x2;
...
template <typename Scalar>
KOKKOS_INLINE_FUNCTION Scalar poly_min_extrap(const Scalar f0, const Scalar d0, const Scalar f1) {
  const Scalar temp = 2 * (f1 - f0 - d0);
  if (Kokkos::abs(temp) <= d0 * Kokkos::Experimental::epsilon_v<Scalar>) {
    return 0.5;
  }

  const Scalar alpha = -d0 / temp;
...
template <typename Scalar>
KOKKOS_INLINE_FUNCTION Scalar poly_min_extrap(const Scalar f0, const Scalar d0, const Scalar x1, const Scalar f_x1,
                                              const Scalar x2, const Scalar f_x2) {
  ...
  const Scalar aa2 = x2 * x2;
  const Scalar aa1 = x1 * x1;
  const Scalar m11 = aa2;
  const Scalar m12 = -aa1;
  const Scalar m21 = -aa2 * x2;
  const Scalar m22 = aa1 * x1;
  const Scalar v1 = f_x1 - f0 - d0 * x1;
  const Scalar v2 = f_x2 - f0 - d0 * x2;

  const Scalar temp1 = aa2 * aa1 * (x1 - x2);
  ...
  const Scalar inv_temp1 = 1.0 / temp1;
  const Scalar a = (m11 * v1 + m12 * v2) * inv_temp1;
  const Scalar b = (m21 * v1 + m22 * v2) * inv_temp1;
  const Scalar temp2 = b * b - 3 * a * d0;
  if (temp2 < 0 || a == 0) {
  ...
  const Scalar temp3 = (-b + Kokkos::sqrt(temp2)) / (3 * a);
  return put_in_range(0, x2, temp3);
}

template <size_t N, typename Scalar, typename CostFunctionType>
class line_search_funct {
 public:
  line_search_funct(const CostFunctionType& const_func, const Vector<Scalar, N>& start,
                    const Vector<Scalar, N>& direction)
 ...
 private:
  ...
  const Vector<Scalar, N>& start_;
  const Vector<Scalar, N>& direction_;
};

/// \brief Factory function for line_search_funct
template <size_t N, typename CostFunctionType, typename Scalar>
auto make_line_search_funct(const CostFunctionType& cost_func, const Vector<Scalar, N>& start,
                            const Vector<Scalar, N>& direction) {
  return line_search_funct<N, Scalar, CostFunctionType>(cost_func, start, direction);
}

template <typename Scalar>
class objective_delta_stop_strategy {
 public:
  explicit objective_delta_stop_strategy(const Scalar min_delta = 1e-7)
      : been_used_(false), min_delta_(min_delta), max_iter_(0), cur_iter_(0), prev_funct_value_(0) {
  }

  objective_delta_stop_strategy(Scalar min_delta, size_t max_iter)
      : been_used_(false), min_delta_(min_delta), max_iter_(max_iter), cur_iter_(0), prev_funct_value_(0) {
  }

  template <size_t N>
  bool should_continue_search(const Vector<Scalar, N>&, const Scalar funct_value, const Vector<Scalar, N>&) {
    ++cur_iter_;
    if (been_used_) {
 ...
 private:
  ...
  size_t max_iter_;
  size_t cur_iter_;
  Scalar prev_funct_value_;
};

template <size_t N, typename CostFunctionType, typename Scalar>
class central_differences {
 public:
  explicit central_differences(const CostFunctionType& const_func, Scalar eps = 1e-7)
      : const_func_(const_func), eps_(eps) {
  }

  Vector<Scalar, N> operator()(const Vector<Scalar, N>& x) const {
    Vector<Scalar, N> der;
    Vector<Scalar, N> e = x;
    for (size_t i = 0; i < N; ++i) {
      const Scalar old_val = e[i];

      e[i] += eps_;
      const Scalar delta_plus = const_func_(e);
      e[i] = old_val - eps_;
      const Scalar delta_minus = const_func_(e);
...
/// \brief Factory function for central_differences
template <size_t N, typename CostFunctionType, typename Scalar>
auto make_central_differences(const CostFunctionType& cost_func, Scalar eps) {
  return central_differences<N, CostFunctionType, Scalar>(cost_func, eps);
}

template <typename CostFunctionType, typename CostFunctionDerType, typename Scalar>
KOKKOS_FUNCTION Scalar line_search(const CostFunctionType& f, const Scalar f0, const CostFunctionDerType& der,
                                   const Scalar d0, const Scalar rho, const Scalar sigma, const Scalar min_f,
  ...
  // the search.
  const Scalar tau1a = 1.4;
  const Scalar tau1b = 9;
  ...
  const Scalar tau2 = 1.0 / 10.0;
  const Scalar tau3 = 1.0 / 2.0;

  // Stop right away and return a step size of 0 if the gradient is 0 at the starting point
  if (Kokkos::abs(d0) <= Kokkos::abs(f0) * Kokkos::Experimental::epsilon_v<Scalar>) {
    return 0;
  }
  ...
  const Scalar mu = (min_f - f0) / (rho * d0);
  Scalar alpha = 1;
  if (mu < 0) {
    alpha = -alpha;
  }
  ...
  Scalar a_val, b_val, a_val_der, b_val_der;

  // This thresh value represents the Wolfe curvature condition
  const Scalar thresh = Kokkos::abs(sigma * d0);

  size_t itr = 0;
  // do the bracketing stage to find the bracket range [a,b]
  while (true) {
    ++itr;
    const Scalar val = f(alpha);
    const Scalar val_der = der(alpha);
  ...
  while (true) {
    ++itr;
    Scalar first = a + tau2 * (b - a);
    Scalar last = b - tau3 * (b - a);
    ...
    // alpha.
    const Scalar max_possible_alpha = Kokkos::max(Kokkos::abs(a), Kokkos::abs(b));
    if (Kokkos::abs(max_possible_alpha * d0) <= Kokkos::abs(f0) * Kokkos::Experimental::epsilon_v<Scalar>) {
      return alpha;
    }
...
template <size_t max_size, typename Scalar, size_t N>
class lbfgs_search_strategy {
 public:
  lbfgs_search_strategy() : been_used(false), current_size(0) {
  ...
  Scalar get_wolfe_rho() const {
    return 0.01;
  }

  Scalar get_wolfe_sigma() const {
    return 0.9;
  }
  ...
  const Vector<Scalar, N>& get_next_direction(const Vector<Scalar, N>& x, const Scalar,
                                              const Vector<Scalar, N>& funct_derivative) {
    ...
    if (!been_used) {
      ...
      Scalar temp = dot(dh_temp.s, dh_temp.y);
      // only accept this bit of data if temp isn't zero
      if (Kokkos::abs(temp) > Kokkos::Experimental::epsilon_v<Scalar>) {
      ...
      if (current_size > 0) {
        ...
        Scalar H_0 = 1.0 / data[current_size - 1].rho / dot(data[current_size - 1].y, data[current_size - 1].y);
        H_0 = put_in_range(0.001, 1000.0, H_0);
        prev_direction = H_0 * prev_direction;

        for (size_t i = 0; i < current_size; ++i) {
          Scalar beta = data[i].rho * dot(data[i].y, prev_direction);
 ...
 private:
  ...
  Vector<Scalar, max_size> alpha;

  bool been_used;
  size_t current_size;
  Vector<Scalar, N> prev_x;
  Vector<Scalar, N> prev_derivative;
  Vector<Scalar, N> prev_direction;
  data_helper dh_temp;
...
template <size_t max_size, typename Scalar, size_t N, typename search_strategy_type, typename stop_strategy_type,
          typename CostFunctionType>
KOKKOS_FUNCTION Scalar find_min_using_approximate_derivatives(search_strategy_type search_strategy,
                                                              stop_strategy_type stop_strategy,
                                                              const CostFunctionType& cost_func, Vector<Scalar, N>& x,
                                                              const Scalar min_alowable_cost,
                                                              const Scalar derivative_eps = 1e-7) {
  Vector<Scalar, N> g;
  Vector<Scalar, N> s;

  Scalar cost = cost_func(x);
  const auto derivative_func = make_central_differences<N>(cost_func, derivative_eps);
  g = derivative_func(x);
  while (stop_strategy.template should_continue_search<N>(x, cost, g) && cost > min_alowable_cost) {
    s = search_strategy.get_next_direction(x, cost, g);

    Scalar alpha =
        line_search(make_line_search_funct<N>(cost_func, x, s), cost,
                    make_central_differences<N>(make_line_search_funct<N>(cost_func, x, s), derivative_eps),
...
template <size_t max_size, typename Scalar, size_t N, typename CostFunctionType>
KOKKOS_FUNCTION Scalar
find_min_using_approximate_derivatives(const CostFunctionType& cost_func, Vector<Scalar, N>& x,
                                       const Scalar min_alowable_cost = -Kokkos::Experimental::infinity_v<Scalar>,
                                       const Scalar min_objective_delta = 1e-7, const Scalar derivative_eps = 1e-7) {
  auto stop_strategy = impl::objective_delta_stop_strategy(min_objective_delta);
  auto search_strategy = impl::lbfgs_search_strategy<max_size, Scalar, N>();
  return impl::find_min_using_approximate_derivatives<max_size, Scalar, N>(search_strategy, stop_strategy, cost_func, x,
