...
KOKKOS_INLINE_FUNCTION Kokkos::Array<T, N> copy(const Kokkos::Array<T, N>& a) {
...
template <typename T, size_t N>
KOKKOS_INLINE_FUNCTION Kokkos::Array<T, N> negate(const Kokkos::Array<T, N>& a) {
  Kokkos::Array<T, N> result;
  for (size_t i = 0; i < N; ++i) {
    result[i] = -a[i];
  }
...
template <typename T, size_t N>
KOKKOS_INLINE_FUNCTION Kokkos::Array<T, N> subtract(const Kokkos::Array<T, N>& a, const Kokkos::Array<T, N>& b) {
  Kokkos::Array<T, N> result;
  for (size_t i = 0; i < N; ++i) {
    result[i] = a[i] - b[i];
  }
...
template <typename T, size_t N>
KOKKOS_INLINE_FUNCTION Kokkos::Array<T, N> add(const Kokkos::Array<T, N>& a, const Kokkos::Array<T, N>& b) {
  Kokkos::Array<T, N> result;
  for (size_t i = 0; i < N; ++i) {
    result[i] = a[i] + b[i];
  }
...
template <typename T, size_t N>
KOKKOS_INLINE_FUNCTION Kokkos::Array<T, N> scale(const Kokkos::Array<T, N>& a, T scalar) {
  Kokkos::Array<T, N> result;
  for (size_t i = 0; i < N; ++i) {
    result[i] = a[i] * scalar;
  }
...
template <typename T, size_t N>
KOKKOS_INLINE_FUNCTION T dot(const Kokkos::Array<T, N>& a, const Kokkos::Array<T, N>& b) {
  T result = 0.0;
  for (size_t i = 0; i < N; ++i) {
    result += a[i] * b[i];
  }
...
class line_search_funct {
 public:
  line_search_funct(const CostFunctionType& const_func, const Kokkos::Array<double, N>& start,
                    const Kokkos::Array<double, N>& direction)
      : const_func_(const_func), start_(start), direction_(direction) {
  }

  double operator()(const double& x) const {
    return const_func_(add(start_, scale(direction_, x)));
  }

 private:
  const CostFunctionType& const_func_;
  const Kokkos::Array<double, N>& start_;
  const Kokkos::Array<double, N>& direction_;
};
...
class objective_delta_stop_strategy {
 public:
  ...
  template <size_t N>
  bool should_continue_search(const Kokkos::Array<double, N>&, const double funct_value,
                              const Kokkos::Array<double, N>&) {
    ++cur_iter_;
    if (been_used_) {
...
class central_differences {
 public:
  ...
  template <size_t N>
  Kokkos::Array<double, N> operator()(const Kokkos::Array<double, N>& x) const {
    Kokkos::Array<double, N> der;
    Kokkos::Array<double, N> e = copy(x);
...
class lbfgs_search_strategy {
 public:
  ...
  const Kokkos::Array<double, N>& get_next_direction(const Kokkos::Array<double, N>& x, const double,
                                                     const Kokkos::Array<double, N>& funct_derivative) {
    prev_direction = negate(funct_derivative);

    if (!been_used) {
      been_used = true;
    } else {
      // add an element into the stored data sequence
      dh_temp.s = subtract(x, prev_x);
      dh_temp.y = subtract(funct_derivative, prev_derivative);
      ...
      if (current_size > 0) {
        ...
        for (size_t i = current_size - 1; i < current_size; --i) {
          ...
          prev_direction = subtract(prev_direction, scale(data[i].y, alpha[i]));
        }
        ...
        prev_direction = scale(prev_direction, H_0);

        for (size_t i = 0; i < current_size; ++i) {
          double beta = data[i].rho * dot(data[i].y, prev_direction);
          prev_direction = add(prev_direction, scale(data[i].s, alpha[i] - beta));
        }
      }
    }
 ...
 private:
  struct data_helper {
    Kokkos::Array<double, N> s;
    Kokkos::Array<double, N> y;
    double rho;
  };

  Kokkos::Array<data_helper, max_size> data;
  Kokkos::Array<double, max_size> alpha;

  bool been_used;
  size_t current_size;
  Kokkos::Array<double, N> prev_x;
  Kokkos::Array<double, N> prev_derivative;
  Kokkos::Array<double, N> prev_direction;
  data_helper dh_temp;
  ...
  void fill(Kokkos::Array<double, max_size>& arr, double value) {
...
KOKKOS_FUNCTION double find_min_using_approximate_derivatives(
    search_strategy_type search_strategy, stop_strategy_type stop_strategy, const CostFunctionType& cost_func,
    Kokkos::Array<double, N>& x, const double min_alowable_cost, const double derivative_eps = 1e-7) {
  Kokkos::Array<double, N> g;
  Kokkos::Array<double, N> s;
...
KOKKOS_FUNCTION double find_min_using_approximate_derivatives(
    const CostFunctionType& cost_func, Kokkos::Array<double, N>& x,
  ...
  return impl::find_min_using_approximate_derivatives<max_size, N>(search_strategy, stop_strategy, cost_func, x, min_alowable_cost,
                                                      derivative_eps);
}
