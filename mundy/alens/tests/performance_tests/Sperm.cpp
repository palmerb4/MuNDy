...
class ComputeSLTConstraintForce : public mundy::meta::MetaMethodExecutionInterface<void> {
 public:
  ComputeSLTConstraintForce(mundy::mesh::BulkData *const bulk_data_ptr,
                            const Teuchos::ParameterList &fixed_params = Teuchos::ParameterList())
      : bulk_data_ptr_(bulk_data_ptr), meta_data_ptr_(&bulk_data_ptr_->mesh_meta_data()) {
    // The bulk data pointer must not be null.
    MUNDY_THROW_ASSERT(bulk_data_ptr_ != nullptr, std::invalid_argument,
                       "ComputeSLTConstraintForce: bulk_data_ptr cannot be a nullptr.");

    // Validate the input params. Use default values for any parameter not given.
    Teuchos::ParameterList valid_fixed_params = fixed_params;
    valid_fixed_params.validateParametersAndSetDefaults(ComputeSLTConstraintForce::get_valid_fixed_params());

    // Store the ComputeSLTConstraintForce part acton on by this method.,
    ...
    auto field_exists = [](const stk::mesh::FieldBase *field_ptr, const std::string &field_name) {
      MUNDY_THROW_ASSERT(
          ...
          "ComputeSLTConstraintForce: Field " << field_name << " cannot be a nullptr. Check that the field exists.");
    };  // field_exists
  ...
  static std::shared_ptr<PolymorphicBaseType> create_new_instance(
      ...
      const Teuchos::ParameterList &fixed_params = Teuchos::ParameterList()) {
    return std::make_shared<ComputeSLTConstraintForce>(bulk_data_ptr, fixed_params);
  }
  ...
  void set_mutable_params(const Teuchos::ParameterList &mutable_params) override {
    ...
    valid_mutable_params.validateParametersAndSetDefaults(ComputeSLTConstraintForce::get_valid_mutable_params());
    ...
    MUNDY_THROW_ASSERT(youngs_modulus_ > 0.0, std::invalid_argument,
                       "ComputeSLTConstraintForce: Young's modulus must be greater than zero.");
    MUNDY_THROW_ASSERT(poissons_ratio_ > 0.0, std::invalid_argument,
                       "ComputeSLTConstraintForce: Poisson's ratio must be greater than zero.");
    MUNDY_THROW_ASSERT(shear_modulus_ > 0.0, std::invalid_argument,
                       "ComputeSLTConstraintForce: Shear modulus must be greater than zero.");
    MUNDY_THROW_ASSERT(density_ > 0.0, std::invalid_argument,
                       "ComputeSLTConstraintForce: Density must be greater than zero.");
  }
  //@}
  ...
  void execute() {
    ...
    stk::mesh::for_each_entity_run(
        ...
        [&edge_orientation_field, &node_curvature_field, &node_rotation_gradient_field](
          ...
          MUNDY_THROW_ASSERT(bulk_data.num_nodes(slt_element) == 3, std::logic_error,
                             "ComputeSLTConstraintForce: Elements must have exactly 3 nodes.");
          MUNDY_THROW_ASSERT(bulk_data.num_edges(slt_element) == 2, std::logic_error,
                             "ComputeSLTConstraintForce: Elements must have exactly 2 edges.");
    ...
    stk::mesh::for_each_entity_run(
        ...
        [&node_force_field, &&node_curvature_field, &node_rest_curvature_field, &node_rotation_gradient_field,
         &edge_tangent_field, &edge_binormal_field, &edge_length_field,
...
int main(int argc, char **argv) {
  ...
  // ComputeSLTConstraintForce fixed parameters
  Teuchos::ParameterList compute_slt_constraint_force_fixed_params;
  ...
  compute_ssd_and_cn_fixed_params.set<Teuchos::Array<std::string>>(
      "enabled_kernel_names",
      Teuchos::tuple<std::string>(std::string("SPHEROCYLINDER_SEGMENT_SPHEROCYLINDER_SEGMENT_LINKER")));

  // ComputeAABB fixed parameters
  Teuchos::ParameterList compute_aabb_fixed_params;
  compute_aabb_fixed_params.set<Teuchos::Array<std::string>>(
      "enabled_kernel_names", Teuchos::tuple<std::string>(std::string("SPHEROCYLINDER_SEGMENT")));
  ...
  generate_neighbor_linkers_fixed_params.set("enabled_technique_name", "STK_SEARCH")
      .set<Teuchos::Array<std::string>>(
          ...
          Teuchos::tuple<std::string>("SPHEROCYLINDER_SEGMENT_SPHEROCYLINDER_SEGMENT_LINKERS"));
  generate_neighbor_linkers_fixed_params.sublist("STK_SEARCH")
      .set<Teuchos::Array<std::string>>("valid_source_entity_part_names",
                                        Teuchos::tuple<std::string>(std::string("SPHEROCYLINDER_SEGMENTS")))
  ...
  evaluate_linker_potentials_fixed_params.set<Teuchos::Array<std::string>>(
      "enabled_kernel_names",
      Teuchos::tuple<std::string>("SPHEROCYLINDER_SEGMENT_SPHEROCYLINDER_SEGMENT_HERTZIAN_CONTACT"));
  ...
  linker_potential_force_magnitude_reduction_fixed_params.set<Teuchos::Array<std::string>>(
      "enabled_kernel_names", Teuchos::tuple<std::string>("SPHEROCYLINDER_SEGMENT"));
  ...
  declare_and_init_constraints_fixed_params.set("enabled_technique_name", "CHAIN_OF_SPRINGS")
      ...
      .set<Teuchos::Array<std::string>>("spherocylinder_segment_part_names",
  ...
  auto [compute_slt_constraint_force_ptr, compute_constraint_forcing_ptr, compute_ssd_and_cn_ptr, compute_aabb_ptr,
        generate_neighbor_linkers_ptr, evaluate_linker_potentials_ptr, linker_potential_force_magnitude_reduction_ptr,
        destroy_neighbor_linkers_ptr, declare_and_init_constraints_ptr, bulk_data_ptr] =
      mundy::meta::utils::generate_class_instance_and_mesh_from_meta_class_requirements<
          ComputeSLTConstraintForce, mundy::constraints::ComputeConstraintForcing,
          ...
          {compute_slt_constraint_force_fixed_params, compute_constraint_forcing_fixed_params,
  ...
  check_class_instance(compute_slt_constraint_force_ptr, "ComputeSLTConstraintForce");
  ...
  // ComputeSLTConstraintForce mutable parameters
  Teuchos::ParameterList compute_slt_constraint_force_mutable_params;
  compute_slt_constraint_force_mutable_params.set("youngs_modulus", youngs_modulus)
      .set("poissons_ratio", poissons_ratio)
      .set("shear_modulus", youngs_modulus / (2.0 * (1.0 + poissons_ratio)))
      .set("density", 1.0);
  compute_slt_constraint_force_ptr->set_mutable_params(compute_slt_constraint_force_mutable_params);
