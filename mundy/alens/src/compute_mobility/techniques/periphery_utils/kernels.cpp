...
Eigen::MatrixXd kernels::stokeslet_direct_cpu(const Eigen::Ref<const Eigen::MatrixXd> &r_sl,
...
Eigen::MatrixXd kernels::stresslet_direct_cpu(const Eigen::Ref<const Eigen::MatrixXd> &r_sl,
                                              const Eigen::Ref<const Eigen::MatrixXd> &r_dl,
                                              const Eigen::Ref<const Eigen::MatrixXd> &r_trg,
                                              const Eigen::Ref<const Eigen::MatrixXd> &f_sl,
                                              const Eigen::Ref<const Eigen::MatrixXd> &f_dl, double eta) {
...
Eigen::MatrixXd kernels::oseen_tensor_direct(const Eigen::Ref<const Eigen::MatrixXd> &r_src,
  ...
  // # Compute matrix of size 3N \times 3N
  Eigen::MatrixXd G = Eigen::MatrixXd::Zero(r_trg.size(), r_src.size());
...
Eigen::MatrixXd kernels::stresslet_times_normal(const Eigen::Ref<const Eigen::MatrixXd> &r_src,
  ...
  for (int i = 0; i < N; ++i) {
    for (int j = 0; j < N; ++j) {
      ...
      if (r_norm < epsilon_distance) r_norm = sqrt(r_norm * r_norm + reg2);
