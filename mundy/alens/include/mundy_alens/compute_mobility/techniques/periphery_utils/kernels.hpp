...
#ifndef MUNDY_ALENS_COMPUTE_MOBILITY_PERIPHERY_UTILS_KERNELS_HPP_
#define MUNDY_ALENS_COMPUTE_MOBILITY_PERIPHERY_UTILS_KERNELS_HPP_
...
using Evaluator = std::function<Eigen::MatrixXd(
    const Eigen::Ref<const Eigen::MatrixXd> &r_sl, const Eigen::Ref<const Eigen::MatrixXd> &r_dl,
    const Eigen::Ref<const Eigen::MatrixXd> &r_trg, const Eigen::Ref<const Eigen::MatrixXd> &f_sl,
    const Eigen::Ref<const Eigen::MatrixXd> &f_dl, double eta)>;

Eigen::MatrixXd stokeslet_direct_cpu(const Eigen::Ref<const Eigen::MatrixXd> &r_sl,
...
Eigen::MatrixXd stresslet_direct_cpu(const Eigen::Ref<const Eigen::MatrixXd> &r_sl,
                                     const Eigen::Ref<const Eigen::MatrixXd> &r_dl,
                                     const Eigen::Ref<const Eigen::MatrixXd> &r_trg,
                                     const Eigen::Ref<const Eigen::MatrixXd> &f_sl,
                                     const Eigen::Ref<const Eigen::MatrixXd> &f_dl, double eta);
...
Eigen::MatrixXd oseen_tensor_direct(const Eigen::Ref<const Eigen::MatrixXd> &r_src,
                                    const Eigen::Ref<const Eigen::MatrixXd> &r_trg, double eta, double reg = 5E-3,
...
Eigen::MatrixXd rotlet(const Eigen::Ref<const Eigen::MatrixXd> &r_src, const Eigen::Ref<const Eigen::MatrixXd> &r_trg,
                       const Eigen::Ref<const Eigen::MatrixXd> &density, double eta, double reg = 5E-3,
                       double epsilon_distance = 1E-5);

Eigen::MatrixXd stresslet_times_normal(const Eigen::Ref<const Eigen::MatrixXd> &r_src,
                                       const Eigen::Ref<const Eigen::MatrixXd> &normals, double eta, double reg = 5E-3,
                                       double epsilon_distance = 1E-5);
...
class FMM {
 public:
  ...
  FMM(const int order, const int maxPoints, const stkfmm::PAXIS paxis, const stkfmm::KERNEL k, const F &kernel_func)
      : fmmPtr_(new stkfmm_type(order, maxPoints, paxis, static_cast<unsigned>(k))), k_(k), kernel_func_(kernel_func){};

  /// \brief Set flag to force next call to set up tree, regardless of cache variables
  void force_setup_tree() {
    force_setup_tree_ = true;
  };
  ...
  Eigen::MatrixXd operator()(const Eigen::Ref<const Eigen::MatrixXd> &r_sl,
    ...
    if (setup_flag) {
      ...
      fmmPtr_->setPoints(r_sl.size() / 3, r_sl.data(), r_trg.size() / 3, r_trg.data(), r_dl.size() / 3, r_dl.data());
...
#endif  // MUNDY_ALENS_COMPUTE_MOBILITY_PERIPHERY_UTILS_KERNELS_HPP_
