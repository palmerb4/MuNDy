...
/// \brief Definition of the ComputeBoundingRadius's SpherocylinderSegment kernel.
...
SpherocylinderSegment::SpherocylinderSegment(mundy::mesh::BulkData *const bulk_data_ptr,
  ...
  MUNDY_THROW_REQUIRE(
      ...
      "SpherocylinderSegment: element_bounding_radius_field_ptr cannot be a nullptr. Check that the field exists.");
...
void SpherocylinderSegment::execute(const stk::mesh::Selector &spherocylinder_segment_selector) {
  ...
  stk::mesh::for_each_entity_run(
      *bulk_data_ptr_, stk::topology::ELEMENT_RANK, intersection_with_valid_entity_parts,
      [&node_coord_field, &element_radius_field, &element_bounding_radius_field, &buffer_distance](
