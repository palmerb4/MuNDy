...
/// \brief Base class for all SpherocylinderData objects
///
/// This base class simply hold runtime information about the SpherocylinderData object.
class SpherocylinderDataBase {
 public:
  /// \brief Constructor
  ///
  /// \param name The string name identifying an instance of the SpherocylinderData object
  /// \param bulk_data The bulk data object
  /// \param topology The topology of entities within this aggregate
  SpherocylinderDataBase(const std::string& name, const stk::mesh::BulkData& bulk_data, stk::topology topology,
                      const stk::mesh::PartVector& parts)
      : name_(name), bulk_data_(bulk_data), topology_(topology), rank_(topology_.rank()), parts_(parts) {
  }

  /// \brief Get the name of the SpherocylinderData object
 ...
 private:
  const std::string name_;                ///< The name of the SpherocylinderData object
...
/// \brief Aggregate to hold the data for a collection of spherocylinder
///
/// 
///
...
///
/// The tags for the field types are used to control the type of data stored in the SpherocylinderData object.
...
///
/// Use \ref create_spherocylinder_data to build an SpherocylinderData object with automatic template deduction.
template <typename Scalar,       //
          typename OurTopology,  //
          typename CenterDataTag = CENTER_DATA_IS_FIELD, //
typename OrientationDataTag = ORIENTATION_DATA_IS_FIELD, //
typename RadiusDataTag = RADIUS_DATA_IS_FIELD, //
typename LengthDataTag = LENGTH_DATA_IS_FIELD>
class SpherocylinderData : public SpherocylinderDataBase {
  static_assert(OurTopology::value == NODE || OurTopology::value == PARTICLE,
"The topology of an spherocylinder must be either NODE or PARTICLE");
...
using length_data_t = map_tag_to_data_type_t</* Tag */ LengthDataTag, //
                                           /* Scalar */ scalar_t, //
                                           /* Shared type */ scalar_t>;
  ...
  /// \param name The string name identifying an instance of the SpherocylinderData object
  /// \param bulk_data The bulk data object
  /// \param center_data Center of each spherocylinder (NODE_RANK). (shared type: mundy::math::Vector3<scalar_t>)
/// \param orientation_data Orientation of each spherocylinder (SAME_RANK_AS_TOPOLOGY). (shared type: mundy::math::Quaternion<scalar_t>)
/// \param radius_data Radius of each spherocylinder (SAME_RANK_AS_TOPOLOGY). (shared type: scalar_t)
/// \param length_data Length of each spherocylinder (SAME_RANK_AS_TOPOLOGY). (shared type: scalar_t)
  SpherocylinderData(const std::string& name, const stk::mesh::BulkData& bulk_data, const stk::mesh::PartVector& parts,
                  center, orientation, radius, length)
      : SpherocylinderDataBase(name, bulk_data, topology_t, parts), center_data_(center_data_), orientation_data_(orientation_data_), radius_data_(radius_data_), length_data_(length_data_) {
...
if constexpr (std::is_same_v<OrientationDataTag, tag_type::FIELD>) {
  MUNDY_THROW_ASSERT(orientation_data_->entity_rank() == rank_t, std::invalid_argument,
                     "The orientation_data data must be a field of rank_t");
} else if constexpr (std::is_same_v<OrientationDataTag, tag_type::VECTOR_OF_FIELDS>) {
  for (const auto orientation_data_field_ptr_ : orientation_data_) {
    MUNDY_THROW_ASSERT(orientation_data_field_ptr_->entity_rank() == rank_t, std::invalid_argument,
                       "The orientation_data data must be a vector of fields of rank_t");
  }
}
...
if constexpr (std::is_same_v<LengthDataTag, tag_type::FIELD>) {
  MUNDY_THROW_ASSERT(length_data_->entity_rank() == rank_t, std::invalid_argument,
                     "The length_data data must be a field of rank_t");
} else if constexpr (std::is_same_v<LengthDataTag, tag_type::VECTOR_OF_FIELDS>) {
  for (const auto length_data_field_ptr_ : length_data_) {
    MUNDY_THROW_ASSERT(length_data_field_ptr_->entity_rank() == rank_t, std::invalid_argument,
                       "The length_data data must be a vector of fields of rank_t");
  }
}
  }
...
/// \brief Get the tag for the orientation_data
static constexpr OrientationDataTag orientation_data_tag() {
...
/// \brief Get the tag for the length_data
static constexpr LengthDataTag length_data_tag() {
  return LengthDataTag{};
}
...
/// \brief Get the orientation_data
...
const length_data_t& length_data() const {
  ...
  auto get_entity_view(stk::mesh::Entity entity) {
    using our_t = SpherocylinderData<Scalar, OurTopology, CenterDataTag, OrientationDataTag, RadiusDataTag, LengthDataTag>;
    return mundy::geom::create_topological_entity_view<OurTopology::value>(SpherocylinderDataBase::bulk_data(), entity)
  ...
  /// \brief Get the NGP-compatible version of this SpherocylinderData object
  auto get_updated_ngp_data() const {
    return create_ngp_ellipsoid_data<scalar_t, OurTopology>(
        stk::mesh::get_updated_ngp_mesh(SpherocylinderDataBase::bulk_data()),  //
        list_of_tagged_data_to_ngp_placeholder);
  }

 private:
  center_data_t center_data_;  ///< Center of each spherocylinder (NODE_RANK).
orientation_data_t orientation_data_;  ///< Orientation of each spherocylinder (SAME_RANK_AS_TOPOLOGY).
radius_data_t radius_data_;  ///< Radius of each spherocylinder (SAME_RANK_AS_TOPOLOGY).
length_data_t length_data_;  ///< Length of each spherocylinder (SAME_RANK_AS_TOPOLOGY).
};  // SpherocylinderData

/// \brief Base class for all NgpSpherocylinderData objects
///
/// This base class simply hold runtime information about the NgpSpherocylinderData object.
class NgpSpherocylinderDataBase {
 public:
  /// \brief Constructor
  ///
  /// \param ngp_mesh The NgpMesh object
  /// \param topology The topology of entities within this aggregate
  NgpSpherocylinderDataBase(stk::mesh::NgpMesh ngp_mesh, mundy::core::NgpVector<stk::mesh::Part*> parts,
...
using radius_data_t = map_tag_to_ngp_data_type_t</* Tag */ RadiusDataTag, //
                                           /* Scalar */ scalar_t, //
                                           /* Shared type */ scalar_t>;
using length_data_t = map_tag_to_ngp_data_type_t</* Tag */ LengthDataTag, //
                                           /* Scalar */ scalar_t, //
                                           /* Shared type */ scalar_t>;
  static constexpr stk::topology::topology_t topology_t = OurTopology::value;
  static constexpr stk::topology::rank_t rank_t = stk::topology_detail::topology_data<OurTopology::value>::rank;
...
/// \param radius_data Radius of each spherocylinder (SAME_RANK_AS_TOPOLOGY). (shared type: scalar_t)
/// \param length_data Length of each spherocylinder (SAME_RANK_AS_TOPOLOGY). (shared type: scalar_t)
  NgpSpherocylinderData(stk::mesh::NgpMesh ngp_mesh, mundy::core::NgpVector<stk::mesh::Part*> parts,
                     center, orientation, radius, length)
      : NgpSpherocylinderDataBase(ngp_mesh, parts, topology_t), center_data_(center_data_), orientation_data_(orientation_data_), radius_data_(radius_data_), length_data_(length_data_) {
    if constexpr (std::is_same_v<CenterDataTag, tag_type::FIELD>) {
...
if constexpr (std::is_same_v<OrientationDataTag, tag_type::FIELD>) {
  MUNDY_THROW_ASSERT(orientation_data_->get_rank() == rank_t, std::invalid_argument,
                     "The orientation_data data must be a field of rank_t");
} else if constexpr (std::is_same_v<OrientationDataTag, tag_type::VECTOR_OF_FIELDS>) {
  for (const auto orientation_data_field_ptr_ : orientation_data_) {
    MUNDY_THROW_ASSERT(orientation_data_field_ptr_->get_rank() == rank_t, std::invalid_argument,
                       "The orientation_data data must be a vector of fields of rank_t");
  }
}
...
if constexpr (std::is_same_v<LengthDataTag, tag_type::FIELD>) {
  MUNDY_THROW_ASSERT(length_data_->get_rank() == rank_t, std::invalid_argument,
                     "The length_data data must be a field of rank_t");
} else if constexpr (std::is_same_v<LengthDataTag, tag_type::VECTOR_OF_FIELDS>) {
  for (const auto length_data_field_ptr_ : length_data_) {
    MUNDY_THROW_ASSERT(length_data_field_ptr_->get_rank() == rank_t, std::invalid_argument,
                       "The length_data data must be a vector of fields of rank_t");
  }
}
  }
...
const length_data_t& length_data() const {
  ...
  auto get_entity_view(stk::mesh::FastMeshIndex entity_index) {
    using our_t = NgpSpherocylinderData<Scalar, OurTopology, CenterDataTag, OrientationDataTag, RadiusDataTag, LengthDataTag>;
    return mundy::geom::create_ngp_topological_entity_view<OurTopology::value>(NgpSpherocylinderDataBase::ngp_mesh(),
                                                                               entity_index)
...
/// \brief A helper function to create an SpherocylinderData object
///
...
template <typename Scalar,                           // Must be provided
          ...
          CenterDataType, OrientationDataType, RadiusDataType, LengthDataType>  // deduced
auto create_spherocylinder_data(const stk::mesh::BulkData& bulk_data, const stk::mesh::PartVector& parts,
                              const CenterDataType &center_data, const OrientationDataType &orientation_data, const RadiusDataType &radius_data, const LengthDataType &length_data) {
  using CenterDataTag = map_data_type_to_tag_t<CenterDataType>;
using OrientationDataTag = map_data_type_to_tag_t<OrientationDataType>;
using RadiusDataTag = map_data_type_to_tag_t<RadiusDataType>;
using LengthDataTag = map_data_type_to_tag_t<LengthDataType>;
  return SpherocylinderData<Scalar, stk::topology_detail::topology_data<OurTopology>, CenterDataTag, OrientationDataTag, RadiusDataTag, LengthDataTag>{
      bulk_data, parts, center, orientation, radius, length};
}

/// \brief A helper function to create an NgpSpherocylinderData object
/// See the discussion for create_spherocylinder_data for more information. Only difference is NgpFields over Fields.
template <typename Scalar,                           // Must be provided
          stk::topology::topology_t OurTopology,     // Must be provided
          CenterDataType, OrientationDataType, RadiusDataType, LengthDataType>  // deduced
auto create_ngp_spherocylinder_data(const stk::mesh::NgpMesh& ngp_mesh, const CenterDataType &center_data, const OrientationDataType &orientation_data, const RadiusDataType &radius_data, const LengthDataType &length_data) {
  using CenterDataTag = map_ngp_data_type_to_tag_t<CenterDataType>;
using OrientationDataTag = map_ngp_data_type_to_tag_t<OrientationDataType>;
using RadiusDataTag = map_ngp_data_type_to_tag_t<RadiusDataType>;
using LengthDataTag = map_ngp_data_type_to_tag_t<LengthDataType>;
  return NgpSpherocylinderData<Scalar, stk::topology_detail::topology_data<OurTopology>, CenterDataTag, OrientationDataTag, RadiusDataTag, LengthDataTag>{
      ngp_mesh, center, orientation, radius, length};
}
...
template <typename Scalar, typename OurTopology, typename CenterDataTag, typename OrientationDataTag, typename RadiusDataTag, typename LengthDataTag>
auto get_updated_ngp_data(const SpherocylinderData<Scalar, OurTopology, CenterDataTag, OrientationDataTag, RadiusDataTag, LengthDataTag>& data) {
