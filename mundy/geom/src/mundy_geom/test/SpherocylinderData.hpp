...
if constexpr (std::is_same_v<RadiusDataTag, data_tag::FIELD>) {
  MUNDY_THROW_ASSERT(radius_data_->entity_rank() == rank_t, std::invalid_argument,
                     "The radius_data data must be a field of rank_t");
} else if constexpr (std::is_same_v<RadiusDataTag, data_tag::VECTOR_OF_FIELDS>) {
...
if constexpr (std::is_same_v<LengthDataTag, data_tag::FIELD>) {
  MUNDY_THROW_ASSERT(length_data_->entity_rank() == rank_t, std::invalid_argument,
                     "The length_data data must be a field of rank_t");
} else if constexpr (std::is_same_v<LengthDataTag, data_tag::VECTOR_OF_FIELDS>) {
...
static constexpr RadiusDataTag get_radius_data_tag() {
  return RadiusDataTag{};
}

/// \brief Get the tag for the length_data
static constexpr LengthDataTag get_length_data_tag() {
