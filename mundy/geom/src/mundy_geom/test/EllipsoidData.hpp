...
/// \brief Base class for all EllipsoidData objects
///
/// This base class simply hold runtime information about the EllipsoidData object.
class EllipsoidDataBase {
 public:
  /// \brief Constructor
  ///
  /// \param name The string name identifying an instance of the EllipsoidData object
  ...
  /// \brief Get the name of the EllipsoidData object
 ...
 private:
  const std::string name_;                ///< The name of the EllipsoidData object
...
/// \brief Aggregate to hold the data for a collection of ellipsoid
///
...
///
/// The tags for the field types are used to control the type of data stored in the EllipsoidData object.
...
typename RadiiDataTag = RADII_DATA_IS_FIELD>
...
using radii_data_t = map_tag_to_data_type_t</* Tag */ RadiiDataTag, //
                                           /* Scalar */ scalar_t, //
                                           /* Shared type */ mundy::math::Vector3<scalar_t>>;
  ...
  /// \param name The string name identifying an instance of the EllipsoidData object
  /// \param bulk_data The bulk data object
  /// \param center_data Center of each ellipsoid (NODE_RANK). (shared type: mundy::math::Vector3<scalar_t>)
/// \param orientation_data Orientation of each ellipsoid (SAME_RANK_AS_TOPOLOGY). (shared type: mundy::math::Quaternion<scalar_t>)
/// \param radii_data Radii of each ellipsoid (SAME_RANK_AS_TOPOLOGY). (shared type: mundy::math::Vector3<scalar_t>)
  EllipsoidData(const std::string& name, const stk::mesh::BulkData& bulk_data, const stk::mesh::PartVector& parts,
                  center, orientation, radii)
      : EllipsoidDataBase(name, bulk_data, topology_t, parts), center_data_(center_data_), orientation_data_(orientation_data_), radii_data_(radii_data_) {
...
if constexpr (std::is_same_v<RadiiDataTag, tag_type::FIELD>) {
  MUNDY_THROW_ASSERT(radii_data_->entity_rank() == rank_t, std::invalid_argument,
                     "The radii_data data must be a field of rank_t");
} else if constexpr (std::is_same_v<RadiiDataTag, tag_type::VECTOR_OF_FIELDS>) {
  for (const auto radii_data_field_ptr_ : radii_data_) {
    MUNDY_THROW_ASSERT(radii_data_field_ptr_->entity_rank() == rank_t, std::invalid_argument,
                       "The radii_data data must be a vector of fields of rank_t");
  }
}
  }
...
/// \brief Get the tag for the radii_data
static constexpr RadiiDataTag radii_data_tag() {
  return RadiiDataTag{};
}
...
/// \brief Get the radii_data
const radii_data_t& radii_data() const {
  return radii_data_;
}
  ...
  auto get_entity_view(stk::mesh::Entity entity) {
    using our_t = EllipsoidData<Scalar, OurTopology, CenterDataTag, OrientationDataTag, RadiiDataTag>;
  ...
  /// \brief Get the NGP-compatible version of this EllipsoidData object
  auto get_updated_ngp_data() const {
    return create_ngp_ellipsoid_data<scalar_t, OurTopology>(
        stk::mesh::get_updated_ngp_mesh(EllipsoidDataBase::bulk_data()),  //
        list_of_tagged_data_to_ngp_placeholder);
  }

 private:
  center_data_t center_data_;  ///< Center of each ellipsoid (NODE_RANK).
orientation_data_t orientation_data_;  ///< Orientation of each ellipsoid (SAME_RANK_AS_TOPOLOGY).
radii_data_t radii_data_;  ///< Radii of each ellipsoid (SAME_RANK_AS_TOPOLOGY).
};  // EllipsoidData

/// \brief Base class for all NgpEllipsoidData objects
///
/// This base class simply hold runtime information about the NgpEllipsoidData object.
class NgpEllipsoidDataBase {
 public:
  /// \brief Constructor
  ///
  /// \param ngp_mesh The NgpMesh object
  /// \param topology The topology of entities within this aggregate
  NgpEllipsoidDataBase(stk::mesh::NgpMesh ngp_mesh, mundy::core::NgpVector<stk::mesh::Part*> parts,
...
using orientation_data_t = map_tag_to_ngp_data_type_t</* Tag */ OrientationDataTag, //
                                           /* Scalar */ scalar_t, //
                                           /* Shared type */ mundy::math::Quaternion<scalar_t>>;
using radii_data_t = map_tag_to_ngp_data_type_t</* Tag */ RadiiDataTag, //
                                           /* Scalar */ scalar_t, //
                                           /* Shared type */ mundy::math::Vector3<scalar_t>>;
  static constexpr stk::topology::topology_t topology_t = OurTopology::value;
  static constexpr stk::topology::rank_t rank_t = stk::topology_detail::topology_data<OurTopology::value>::rank;
...
/// \param orientation_data Orientation of each ellipsoid (SAME_RANK_AS_TOPOLOGY). (shared type: mundy::math::Quaternion<scalar_t>)
/// \param radii_data Radii of each ellipsoid (SAME_RANK_AS_TOPOLOGY). (shared type: mundy::math::Vector3<scalar_t>)
  NgpEllipsoidData(stk::mesh::NgpMesh ngp_mesh, mundy::core::NgpVector<stk::mesh::Part*> parts,
                     center, orientation, radii)
      : NgpEllipsoidDataBase(ngp_mesh, parts, topology_t), center_data_(center_data_), orientation_data_(orientation_data_), radii_data_(radii_data_) {
    if constexpr (std::is_same_v<CenterDataTag, tag_type::FIELD>) {
...
if constexpr (std::is_same_v<RadiiDataTag, tag_type::FIELD>) {
  MUNDY_THROW_ASSERT(radii_data_->get_rank() == rank_t, std::invalid_argument,
                     "The radii_data data must be a field of rank_t");
} else if constexpr (std::is_same_v<RadiiDataTag, tag_type::VECTOR_OF_FIELDS>) {
  for (const auto radii_data_field_ptr_ : radii_data_) {
    MUNDY_THROW_ASSERT(radii_data_field_ptr_->get_rank() == rank_t, std::invalid_argument,
                       "The radii_data data must be a vector of fields of rank_t");
  }
}
  }
...
const radii_data_t& radii_data() const {
  ...
  auto get_entity_view(stk::mesh::FastMeshIndex entity_index) {
    using our_t = NgpEllipsoidData<Scalar, OurTopology, CenterDataTag, OrientationDataTag, RadiiDataTag>;
    return mundy::geom::create_ngp_topological_entity_view<OurTopology::value>(NgpEllipsoidDataBase::ngp_mesh(),
                                                                               entity_index)
...
template <typename Scalar,                           // Must be provided
          ...
          CenterDataType, OrientationDataType, RadiiDataType>  // deduced
auto create_ellipsoid_data(const stk::mesh::BulkData& bulk_data, const stk::mesh::PartVector& parts,
                              const CenterDataType &center_data, const OrientationDataType &orientation_data, const RadiiDataType &radii_data) {
...
using RadiiDataTag = map_data_type_to_tag_t<RadiiDataType>;
  return EllipsoidData<Scalar, stk::topology_detail::topology_data<OurTopology>, CenterDataTag, OrientationDataTag, RadiiDataTag>{
      bulk_data, parts, center, orientation, radii};
}
...
template <typename Scalar,                           // Must be provided
          stk::topology::topology_t OurTopology,     // Must be provided
          CenterDataType, OrientationDataType, RadiiDataType>  // deduced
auto create_ngp_ellipsoid_data(const stk::mesh::NgpMesh& ngp_mesh, const CenterDataType &center_data, const OrientationDataType &orientation_data, const RadiiDataType &radii_data) {
...
using RadiiDataTag = map_ngp_data_type_to_tag_t<RadiiDataType>;
  return NgpEllipsoidData<Scalar, stk::topology_detail::topology_data<OurTopology>, CenterDataTag, OrientationDataTag, RadiiDataTag>{
      ngp_mesh, center, orientation, radii};
}
...
template <typename Scalar, typename OurTopology, typename CenterDataTag, typename OrientationDataTag, typename RadiiDataTag>
auto get_updated_ngp_data(const EllipsoidData<Scalar, OurTopology, CenterDataTag, OrientationDataTag, RadiiDataTag>& data) {
