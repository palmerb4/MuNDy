...
if constexpr (std::is_same_v<RadiiDataTag, data_tag::FIELD>) {
  MUNDY_THROW_ASSERT(radii_data_->entity_rank() == rank_t, std::invalid_argument,
                     "The radii_data data must be a field of rank_t");
} else if constexpr (std::is_same_v<RadiiDataTag, data_tag::VECTOR_OF_FIELDS>) {
...
static constexpr RadiiDataTag get_radii_data_tag() {
  return RadiiDataTag{};
}
