...
#ifndef MUNDY_GEOM_AGGREGATES_V_SEGMENTDATA_HPP_
#define MUNDY_GEOM_AGGREGATES_V_SEGMENTDATA_HPP_
...
/// \brief Base class for all VSegmentData objects
///
/// This base class simply hold runtime information about the VSegmentData object.
class VSegmentDataBase {
 public:
  /// \brief Constructor
  ///
  /// \param name The string name identifying an instance of the VSegmentData object
  /// \param bulk_data The bulk data object
  /// \param topology The topology of entities within this aggregate
  VSegmentDataBase(const std::string& name, const stk::mesh::BulkData& bulk_data, stk::topology topology,
                      const stk::mesh::PartVector& parts)
      : name_(name), bulk_data_(bulk_data), topology_(topology), rank_(topology_.rank()), parts_(parts) {
  }

  /// \brief Get the name of the VSegmentData object
 ...
 private:
  const std::string name_;                ///< The name of the VSegmentData object
...
/// \brief Aggregate to hold the data for a collection of v_segment
///
/// 
///
...
///
/// The tags for the field types are used to control the type of data stored in the VSegmentData object.
...
///
/// Use \ref create_v_segment_data to build an VSegmentData object with automatic template deduction.
template <typename Scalar,       //
          typename OurTopology,  //
          typename NodeCoordsDataTag = NODE_COORDS_DATA_IS_FIELD>
class VSegmentData : public VSegmentDataBase {
  static_assert(OurTopology::value == SPRING_3 || OurTopology::value == TRI_3 || OurTopology::value == SHELL_TRI_3,
"The topology of an v_segment must be either SPRING_3 or TRI_3 or SHELL_TRI_3");
 ...
 public:
  ...
  /// \param name The string name identifying an instance of the VSegmentData object
  /// \param bulk_data The bulk data object
  /// \param node_coords_data Coordinates of the nodes of each V segment (NODE_RANK). (shared type: std::vector<mundy::math::Vector3<scalar_t>>)
  VSegmentData(const std::string& name, const stk::mesh::BulkData& bulk_data, const stk::mesh::PartVector& parts,
                  node_coords)
      : VSegmentDataBase(name, bulk_data, topology_t, parts), node_coords_data_(node_coords_data_) {
    if constexpr (std::is_same_v<NodeCoordsDataTag, tag_type::FIELD>) {
...
} else if constexpr (std::is_same_v<NodeCoordsDataTag, tag_type::VECTOR_OF_FIELDS>) {
...
static constexpr NodeCoordsDataTag node_coords_data_tag() {
...
const node_coords_data_t& node_coords_data() const {
  ...
  auto get_entity_view(stk::mesh::Entity entity) {
    using our_t = VSegmentData<Scalar, OurTopology, NodeCoordsDataTag>;
    return mundy::geom::create_topological_entity_view<OurTopology::value>(VSegmentDataBase::bulk_data(), entity)
  ...
  /// \brief Get the NGP-compatible version of this VSegmentData object
  auto get_updated_ngp_data() const {
    return create_ngp_ellipsoid_data<scalar_t, OurTopology>(
        stk::mesh::get_updated_ngp_mesh(VSegmentDataBase::bulk_data()),  //
        list_of_tagged_data_to_ngp_placeholder);
  }

 private:
  node_coords_data_t node_coords_data_;  ///< Coordinates of the nodes of each V segment (NODE_RANK).
};  // VSegmentData

/// \brief Base class for all NgpVSegmentData objects
///
/// This base class simply hold runtime information about the NgpVSegmentData object.
class NgpVSegmentDataBase {
 public:
  /// \brief Constructor
  ///
  /// \param ngp_mesh The NgpMesh object
  /// \param topology The topology of entities within this aggregate
  NgpVSegmentDataBase(stk::mesh::NgpMesh ngp_mesh, mundy::core::NgpVector<stk::mesh::Part*> parts,
...
"The topology of an v_segment must be either SPRING_3 or TRI_3 or SHELL_TRI_3");
 ...
 public:
  ...
  /// \param node_coords_data Coordinates of the nodes of each V segment (NODE_RANK). (shared type: std::vector<mundy::math::Vector3<scalar_t>>)
  NgpVSegmentData(stk::mesh::NgpMesh ngp_mesh, mundy::core::NgpVector<stk::mesh::Part*> parts,
                     node_coords)
      : NgpVSegmentDataBase(ngp_mesh, parts, topology_t), node_coords_data_(node_coords_data_) {
    if constexpr (std::is_same_v<NodeCoordsDataTag, tag_type::FIELD>) {
...
const node_coords_data_t& node_coords_data() const {
  ...
  auto get_entity_view(stk::mesh::FastMeshIndex entity_index) {
    using our_t = NgpVSegmentData<Scalar, OurTopology, NodeCoordsDataTag>;
    return mundy::geom::create_ngp_topological_entity_view<OurTopology::value>(NgpVSegmentDataBase::ngp_mesh(),
                                                                               entity_index)
...
/// \brief A helper function to create an VSegmentData object
///
...
auto create_v_segment_data(const stk::mesh::BulkData& bulk_data, const stk::mesh::PartVector& parts,
                              const NodeCoordsDataType &node_coords_data) {
  using NodeCoordsDataTag = map_data_type_to_tag_t<NodeCoordsDataType>;
  return VSegmentData<Scalar, stk::topology_detail::topology_data<OurTopology>, NodeCoordsDataTag>{
      bulk_data, parts, node_coords};
}

/// \brief A helper function to create an NgpVSegmentData object
/// See the discussion for create_v_segment_data for more information. Only difference is NgpFields over Fields.
template <typename Scalar,                           // Must be provided
          stk::topology::topology_t OurTopology,     // Must be provided
          NodeCoordsDataType>  // deduced
auto create_ngp_v_segment_data(const stk::mesh::NgpMesh& ngp_mesh, const NodeCoordsDataType &node_coords_data) {
  using NodeCoordsDataTag = map_ngp_data_type_to_tag_t<NodeCoordsDataType>;
  return NgpVSegmentData<Scalar, stk::topology_detail::topology_data<OurTopology>, NodeCoordsDataTag>{
...
auto get_updated_ngp_data(const VSegmentData<Scalar, OurTopology, NodeCoordsDataTag>& data) {
...
#endif  // MUNDY_GEOM_AGGREGATES_V_SEGMENTDATA_HPP_
