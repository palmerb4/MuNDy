...
TEST(ComputeAABBStaticInterface, IsRegisterable) {
  ...
  ASSERT_TRUE(HasMeshRequirementsAndIsRegisterable<ComputeAABB>::value);
}
...
TEST(ComputeAABBStaticInterface, FixedParameterDefaults) {
  ...
  ComputeAABB::validateFixedParametersAndSetDefaults(&fixed_params);
  ASSERT_TRUE(fixed_params.isSublist("kernels"));
  Teuchos::ParameterList &kernels_sublist = fixed_params.sublist("kernels", true);
  ASSERT_TRUE(fixed_params.isParameter("count"));
  ASSERT_EQ(fixed_params.get<unsigned>("count"), ComputeAABB::OurKernelFactory::num_registered_classes());
  int i = 0;
  for (auto &key : ComputeAABB::OurKernelFactory::get_keys()) {
    ASSERT_EQ(kernels_sublist.isSublist("kernel_" + std::string(i)));
...
TEST(ComputeAABBStaticInterface, MutableParameterDefaults) {
  ...
  ComputeAABB::validateMutableParametersAndSetDefaults(&mutable_params);
  ASSERT_TRUE(mutable_params.isSublist("kernels"));
  Teuchos::ParameterList &kernels_sublist = mutable_params.sublist("kernels", true);
  ASSERT_TRUE(mutable_params.isParameter("count"));
  ASSERT_EQ(mutable_params.get<unsigned>("count"), ComputeAABB::OurKernelFactory::num_registered_classes());
  int i = 0;
  for (auto &key : ComputeAABB::OurKernelFactory::get_keys()) {
    ASSERT_EQ(kernels_sublist.isSublist("kernel_" + std::string(i)));
    Teuchos::ParameterList &kernel_params = kernels_sublist.sublist("kernel_" + std::to_string(i), true);
...
TEST(ComputeAABBStaticInterface, GetMeshRequirementsFromDefaultParameters) {
  ...
  Teuchos::ParameterList fixed_params;
  ComputeAABB::validateFixedParametersAndSetDefaults(&fixed_params);
  ASSERT_NO_THROW(ComputeAABB::getMeshRequirements(fixed_params));
}
...
TEST(ComputeAABBStaticInterface, CreateNewInstanceFromDefaultParameters) {
  ...
  ComputeAABB::validateFixedParametersAndSetDefaults(&fixed_params);
  ASSERT_NO_THROW(ComputeAABB::getMeshRequirements(fixed_params));
  mesh_reqs_ptr->merge(ComputeAABB::getMeshRequirements(fixed_params));
  ...
  ASSERT_NO_THROW(ComputeAABB::createNewInstance(fixed_params, bulk_data_ptr.get());)
}
//@}
...
TEST(ComputeAABB, PerformsAABBCalculationCorrectlyForSphere) {
  ...
  */
  ...
  Teuchos::ParameterList fixed_params;
  auto mesh_reqs_ptr = std::make_shared<MeshRequirements>(MPI_COMM_WORLD);
  mesh_reqs_ptr->set_space_dimension(3);
  ComputeAABB::validateFixedParametersAndSetDefaults(&fixed_params);
  mesh_reqs_ptr->merge(ComputeAABB::getMeshRequirements(fixed_params));
  std::shared_ptr<mundy::mesh::BulkData> bulk_data_ptr = mesh_reqs_ptr->declare_mesh();
  std::shared_ptr<mundy::mesh::MetaData> meta_data_ptr = bulk_data_ptr->mesh_meta_data().get_meta_data();

  // Create a new instance of ComputeAABB.
  auto compute_aabb_ptr = ComputeAABB::createNewInstance(fixed_params, bulk_data_ptr.get());
  ...
  stk::mesh::Entity sphere_entity = bulk_data_ptr->declare_element(sphere_id, *sphere_part_ptr);
