 ...
 public:
  ...
  static std::vector<std::shared_ptr<mundy::meta::PartRequirements>> details_static_get_part_requirements(
    ...
    for (size_t i = 0; i < num_parts; i++) {
      ...
      part_requirements[i]->set_part_rank(stk::topology::CONSTRAINT_RANK);
      ...
      part_requirements[i]->merge(mundy::meta::MetaKernelFactory<void, MapSurfaceForceToRigidBodyForce>::get_part_requirements(
  ...
  static Teuchos::ParameterList details_static_get_valid_params() {
    ...
    kernel_params.sublist("map_surface_force_to_rigid_body_force", false, "Sublist that defines the map's kernel parameters.");
