...
class ComputeBoundingRadius : public mundy::meta::MetaMethod<ComputeBoundingRadius, void>,
                              public mundy::meta::MetaMethodRegistry<ComputeBoundingRadius> {
 public:
  ...
  static std::vector<std::unique_ptr<PartRequirements>> details_get_part_requirements(
    ...
    for (int i = 0; i < num_parts_; i++) {
      ...
      part_kernel_parameter_list.validateParametersAndSetDefaults(
          MetaKernelFactory<ComputeBoundingRadius>::get_valid_params(kernel_name));

      // Merge the kernel requirements.
      part_requirements[i]->merge(
          &MetaKernelFactory<ComputeBoundingRadius>::get_part_requirements(kernel_name, part_kernel_parameter_list));
    }
 ...
 private:
  ...
  std::vector<std::shared_ptr<MetaKernelBase>> compute_bounding_sphere_kernels_;
  //@}
