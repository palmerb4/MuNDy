...
class ResolveConstraints : public mundy::meta::MetaMethod<void> {
 public:
  //! \name Typedefs
  //@{
  ...
  static void validate_fixed_parameters_and_set_defaults(
    ...
    if (technique_params.isParameter("name")) {
      ...
      MUNDY_THROW_ASSERT(
          ...
          "ResolveConstraints: Type error. Given a parameter with name 'name' but with a type other than std::string");
    } else {
 ...
 private:
  ...
  static constexpr std::string_view default_technique_name_ = "NON_SMOOTH_LCP";
  //@}
