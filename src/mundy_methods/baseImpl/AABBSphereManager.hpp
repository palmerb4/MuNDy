...
#ifndef MUNDY_METHODS_AABBSPHEREMANAGER_HPP_
#define MUNDY_METHODS_AABBSPHEREMANAGER_HPP_

/// \file AABBSphereManager.hpp
/// \brief Declaration of the AABBSphereManager class
...
/// \brief Concrete implementation of AABBManager for spheres.
///
/// This class computes the axis-aligned boundary box for each sphere in a part of spheres. The output field name is
/// specified in the parameter file.
class AABBSphereManager : AABBManager {
 public:
  //! \name Constructors and destructor
  //@{

  /// \brief Constructor
  explicit AABBSphereManager(const stk::util::ParameterList &parameter_list)
      : parameter_list_(parameter_list), aabb_field_name_(params.get_value<std::string>("AABB field name"),
  ...
  static std::unique_ptr<PartParams> get_part_requirements(
    ...
    required_part_params->add_field_params("aabb", std::move(element_double_vector_4d_field_params));
 ...
 private:
  ...
  const stk::string &aabb_field_name_;
...
#endif  // MUNDY_METHODS_AABBSPHEREMANAGER_HPP_
