...
ComputeConstraintProjection::ComputeConstraintProjection(mundy::mesh::BulkData *const bulk_data_ptr,
  ...
  valid_params.validateParametersAndSetDefaults(this->get_valid_params());
  ...
  for (size_t i = 0; i < num_parts_; i++) {
    ...
    const Teuchos::ParameterList &part_kernel_params =
        part_params.sublist("kernels").sublist("compute_constraint_projection");

    // Create the kernel instance.
    const std::string kernel_name = part_kernel_params.get<std::string>("name");
    compute_constraint_projection_kernel_ptrs_.push_back(
        mundy::meta::MetaKernelFactory<void, ComputeConstraintProjection>::create_new_instance(
    ...
    for (size_t j = 0; j < num_parts_; j++) {
      if (i != j) {
        ...
        TEUCHOS_TEST_FOR_EXCEPTION(parts_intersect, std::invalid_argument,
                                   "ComputeConstraintProjection: Part " << part_ptr_vector_[i]->name() << " and "
...
Teuchos::ParameterList ComputeConstraintProjection::set_mutable_params(
  ...
  for (size_t i = 0; i < num_parts_; i++) {
    ...
    stk::mesh::for_each_entity_run(
        *bulk_data_ptr_, stk::topology::ELEM_RANK, locally_owned_part,
        [&compute_constraint_projection_kernel_ptr]([[maybe_unused]] const mundy::mesh::BulkData &bulk_data,
