...
Collision::Collision(stk::mesh::BulkData *const bulk_data_ptr, const Teuchos::ParameterList &parameter_list)
  ...
  signed_sep_dist_field_name_ = valid_parameter_list.get<std::string>("signed_separation_dist_field_name");
  ...
  separation_dist_field_ptr_ =
      meta_data_ptr_->get_field<double>(stk::topology::ELEM_RANK, signed_separation_dist_field_name_);
...
void Collision::execute(const stk::mesh::Entity &element) {
  ...
  const double *separation_dist = stk::mesh::field_data(*separation_dist_field_ptr_, element);
  const double *lagrange_mult = stk::mesh::field_data(*lagrange_multiplier_field_ptr_, element);
