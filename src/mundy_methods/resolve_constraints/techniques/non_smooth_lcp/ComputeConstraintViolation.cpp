...
#include <mundy_methods/resolve_constraints/techniques/non_smooth_lcp/compute_constraint_violation/kernels/AllKernels.hpp>  // performs the registration of all kernels
...
ComputeConstraintViolation::ComputeConstraintViolation(mundy::mesh::BulkData *const bulk_data_ptr,
...
void ComputeConstraintViolation::set_mutable_params(const Teuchos::ParameterList &mutable_params) {
  // Validate the input params. Use default values for any parameter not given.
  ...
  MUNDY_THROW_ASSERT(num_multibody_types_ == kernels_sublist.get<unsigned>("count"), std::invalid_argument,
                     "ComputeConstraintViolation: Internal error. Mismatch between the stored kernel count and "
                     "the parameter list kernel count.\n"
...
void ComputeConstraintViolation::setup() {
}
...
void ComputeConstraintViolation::finalize() {
}
//}
