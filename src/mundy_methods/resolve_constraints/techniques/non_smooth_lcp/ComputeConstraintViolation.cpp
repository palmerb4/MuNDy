...
ComputeConstraintViolation::ComputeConstraintViolation(mundy::mesh::BulkData *const bulk_data_ptr,
  ...
  for (size_t i = 0; i < num_parts_; i++) {
    ...
    const Teuchos::ParameterList &part_kernel_params =
        part_params.sublist("kernels").sublist("compute_constraint_violation");
...
Teuchos::ParameterList ComputeConstraintViolation::set_mutable_params(
  ...
  for (size_t i = 0; i < num_parts_; i++) {
    ...
    stk::mesh::for_each_entity_run(
        *bulk_data_ptr_, stk::topology::ELEM_RANK, locally_owned_part,
        [&compute_constraint_violation_kernel_ptr]([[maybe_unused]] const mundy::mesh::BulkData &bulk_data,
