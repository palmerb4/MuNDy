...
MapSurfaceForceToRigidBodyForce::MapSurfaceForceToRigidBodyForce(stk::mesh::BulkData *const bulk_data_ptr,
  ...
  for (size_t i = 0; i < num_parts_; i++) {
    ...
    kernel_ptrs_.push_back(mundy::meta::MetaKernelFactory<void, MapSurfaceForceToRigidBodyForce>::create_new_instance(
    ...
    for (size_t j = 0; j < num_parts_; j++) {
      if (i != j) {
        ...
        TEUCHOS_TEST_FOR_EXCEPTION(parts_intersect, std::invalid_argument,
                                   "MapSurfaceForceToRigidBodyForce: Part " << part_ptr_vector_[i]->name() << " and "
...
void MapSurfaceForceToRigidBodyForce::execute() {
  ...
  for (size_t i = 0; i < num_parts_; i++) {
    ...
    stk::mesh::for_each_entity_run(*bulk_data_ptr_, stk::topology::ELEM_RANK, locally_owned_part,
                                   [&kernel_ptr]([[maybe_unused]] const stk::mesh::BulkData &bulk_data,
                                                 stk::mesh::Entity linker) { kernel_ptr->execute(linker); });
  }
}
//}
