...
class MultibodyFactory {
 public:
  ...
  static std::unique_ptr<PartParams> get_part_requirements(const mundy::multibody& multibody_type,
    ...
    switch (multibody_type) {
      case mundy::multibody::SPHERE:
        return MultibodySphereManager.get_part_requirements(parameter_list);
      case mundy::multibody::SPHEROCYLINDER:
        return MultibodySphereocylinderManager.get_part_requirements(parameter_list);
      case mundy::multibody::SUPERELLIPSOID:
        return MultibodySuperellipsoidManager.get_part_requirements(parameter_list);
      case mundy::multibody::POLYTOPE:
        return MultibodyPolytopeManager.get_part_requirements(parameter_list);
      case mundy::multibody::COLLISION:
        return MultibodyCollisionManager.get_part_requirements(parameter_list);
      case mundy::multibody::SPRING:
        return MultibodySpringManager.get_part_requirements(parameter_list);
      case mundy::multibody::TORSIONALSPRING:
        return MultibodyTorsionalSpringManager.get_part_requirements(parameter_list);
      case mundy::multibody::JOINT:
        return MultibodyJointManager.get_part_requirements(parameter_list);
      case mundy::multibody::HINGE:
        return MultibodyHingeManager.get_part_requirements(parameter_list);
    }
  }
  ...
  std::unique_ptr<mundy::methods::MultibodyManager> make_subclass(const mundy::multibody& given_type,
    ...
    switch (given_type) {
      case mundy::multibody::SPHERE:
        return std::make_unique<MultibodySphereManager>();
      case mundy::multibody::SPHEROCYLINDER:
        return std::make_unique<MultibodySphereocylinderManager>();
      case mundy::multibody::SUPERELLIPSOID:
        return std::make_unique<MultibodySuperellipsoidManager>();
      case mundy::multibody::POLYTOPE:
        return std::make_unique<MultibodyPolytopeManager>();
      case mundy::multibody::COLLISION:
        return std::make_unique<MultibodyCollisionManager>();
      case mundy::multibody::SPRING:
        return std::make_unique<MultibodySpringManager>();
      case mundy::multibody::TORSIONALSPRING:
        return std::make_unique<MultibodyTorsionalSpringManager>();
      case mundy::multibody::JOINT:
        return std::make_unique<MultibodyJointManager>();
      case mundy::multibody::HINGE:
        return std::make_unique<MultibodyHingeManager>();
    }
  }
