...
#ifndef MUNDY_META_DEFAULTHINGEPARTPARAMS_HPP_
#define MUNDY_META_DEFAULTHINGEPARTPARAMS_HPP_

/// \file DefaultHingePartParams.hpp
/// \brief Declaration of the DefaultHingePartParams class
...
class DefaultHingePartParams {
 public:
  ...
  /// <tt>BulkData</tt> can be shared between each <tt>DefaultHingePartParams</tt>, thereby allowing a <tt>Field</tt> or
  ...
  DefaultHingePartParams(const std::shared_ptr<stk::mesh::BulkData> &bulk_data_ptr, const std::string &group_name);
  //@}
  ...
  /// \brief Declare another <tt>DefaultHingePartParams</tt> as a subset of this group.
  ///
  ...
  void declare_subgroup(const DefaultHingePartParams<SubGroupTopology, SubGroupScalar> &subgroup);
...
DefaultHingePartParams<GroupTopology, Scalar>::DefaultHingePartParams(const std::shared_ptr<stk::mesh::BulkData> &bulk_data_ptr,
    ...
    : bulk_data_ptr_(bulk_data_ptr),
      ...
      new_entity_flag_field_(
          bulk_data_ptr_->mesh_meta_data().declare_field<bool>(DefaultHingePartParams.rank(), "new_entity_flag")) {
...
//{
template <stk::topology GroupTopology, typename Scalar>
FlagFieldType &DefaultHingePartParams<GroupTopology, Scalar>::get_group_part() const {
  return group_part_;
}

template <stk::topology GroupTopology, typename Scalar>
FlagFieldType &DefaultHingePartParams<GroupTopology, Scalar>::get_new_entity_flag_field() const {
  return new_entity_flag_field_;
}

template <stk::topology GroupTopology, typename Scalar>
stk::mesh::Selector DefaultHingePartParams<GroupTopology, Scalar>::get_entity_selector() const {
  return entity_selector_;
}

template <stk::topology GroupTopology, typename Scalar>
stk::mesh::BucketVector &DefaultHingePartParams<GroupTopology, Scalar>::get_entity_buckets() const {
  return bulk_data_ptr_->get_buckets(GroupTopology.rank(), selectLocalParticles);
}

template <stk::topology GroupTopology, typename Scalar>
stk::mesh::BucketVector &DefaultHingePartParams<GroupTopology, Scalar>::get_entity_buckets(
...
template <stk::topology GroupTopology, typename Scalar>
template <stk::topology SubGroupTopology, typename SubGroupScalar>
void DefaultHingePartParams<GroupTopology, Scalar>::declare_subgroup(
    const DefaultHingePartParams<SubGroupTopology, SubGroupScalar> &subgroup) {
...
template <class field_type>
field_type &DefaultHingePartParams<GroupTopology, Scalar>::put_field_on_entire_group(
...
//{
template <stk::topology GroupTopology, typename Scalar>
stk::mesh::Selector DefaultHingePartParams<GroupTopology, Scalar>::generate_new_entities_in_group(
  ...
  const unsigned num_nodes_per_entity = DefaultHingePartParams.num_nodes();
  ...
  for (int i = 0; i < num_particles_local; i++) {
    ...
    if (generate_and_attach_nodes) {
      // attach each node
      for (int j = 0; j < DefaultHingePartParams.num_nodes(); j++) {
...
//}
#endif  // MUNDY_META_DEFAULTHINGEPARTPARAMS_HPP_
